...
const { validateOrigin, validateSignature, generateIndexedHashes, generateRawValues, generateLogKey, evaluateBucketFilter, buildLogsSearchQuery, HToMs } = require("../factory/log");
...
const createLog = async (bucket, params) => {
    ...
    const shouldBeInThisBucket = evaluateBucketFilter(params, bucket?.settings?.filter ?? []);
    if (!shouldBeInThisBucket) {
        return null
    }
